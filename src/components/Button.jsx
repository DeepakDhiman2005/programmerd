...
const Button = ({children, color=false, className=false, onClick=function(){}, value="" , ariaValueText=""}) => {
    return <>
        {
            color === "green" ? <>
                <button className={"p-2 text-white bg-green-600 cursor-pointer border border-solid border-green-600 rounded-md shadow-md shadow-slate-50 hover:bg-green-700"+" "+className} onClick={onClick} value={value} aria-valuetext={ariaValueText}>{children}</button>
            </>:
            color === "red" ? <>
                <button className={"p-2 text-white bg-red-600 cursor-pointer border border-solid border-red-600 rounded-md shadow-md shadow-slate-50 hover:bg-red-700"+" "+className} onClick={onClick} value={value} aria-valuetext={ariaValueText}>{children}</button>
            </>:
            color === "blue" ? <>
