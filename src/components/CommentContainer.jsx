                ...
                {/* comment box */}
                {
                    _data.map((data)=>{
                        return  <div className="w-full md:w-3/4 flex justify-start mt-5 mb-5 items-center">
                            ...
                            <div className="bg-white border border-solid border-slate-200 rounded-lg shadow-md flex flex-col justify-center items-start p-4 w-full sm:w-[90%]">
                                <div className="flex justify-between items-center mb-2 w-full">
                                    <h2 className="text-xl font-bold text-slate-800">{data.name}</h2>
                                    <h2>{data.date}</h2>
                                </div>
                                <p>{data.comment}</p>
                                <button className="text-white mt-3 mb-3 text-sm font-semibold bg-purple-600 cursor-pointer p-2 pt-1 pb-1 border border-solid border-purple-700 rounded-sm hover:bg-purple-800 hover:shadow-lg active:bg-purple-400 active:text-purple-600 selection:text-white">REPLY</button>
                            </div>
                        </div>
                    })
                }
