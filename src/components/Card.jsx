...
const Card = ({image, title="", desc="", button="", height, width, href, link=null}, key="") => {
    return <>
        <div key={key} className="relative flex flex-col bg-clip-border rounded-xl ml-2 mr-2 bg-white text-gray-700 shadow-md mt-6  mb-6 w-96">
            ...
            <div className="p-6 pt-0">
                {
                    link === null  ? <a href={href ? href: null} target="_blank">
                    ...
                    <Link href={link}>
                    <button className="align-middle select-none font-sans font-bold text-center uppercase transition-all disabled:opacity-50 disabled:shadow-none disabled:pointer-events-none text-xs py-3 px-6 rounded-lg bg-gray-900 text-white shadow-md shadow-gray-900/10 hover:shadow-lg hover:shadow-gray-900/20 focus:shadow-none active:opacity-[0.85] active:shadow-none" type="button">{button}</button></Link>
                }
            </div>
        </div>
    </>
